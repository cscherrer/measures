...
fn main() {
    ...
    let _naive_large = iid_normal.iid_log_density(&large_samples);
    let naive_time = start.elapsed();

    // Time efficient approach
    let start = std::time::Instant::now();
    let _efficient_large = iid_normal.iid_log_density(&large_samples);
    ...
    let naive_poisson: f64 = iid_poisson.iid_log_density(&poisson_samples);
    let efficient_poisson: f64 = iid_poisson.iid_log_density(&poisson_samples);
