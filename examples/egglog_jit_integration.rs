...
use measures::exponential_family::{
    AutoJITExt, egglog_optimizer::EgglogOptimize, symbolic_ir::Expr,
};
...
fn create_simple_expression() -> Expr {
    ...
    Expr::add(Expr::add(x, zero_term), Expr::sub(x_pow_1, ln_one))
}
...
fn demonstrate_normal_distribution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Complexity: {}",
        symbolic_log_density.expression.complexity()
    );
    ...
    for &x in &test_points {
        ...
        match (original_result, optimized_result) {
            (Ok(orig), Ok(opt)) => {
                ...
                println!(
                    "     x={x:4.1}: std={standard:8.4}, orig={orig:8.4} (err={original_error:.2e}), opt={opt:8.4} (err={optimized_error:.2e})"
                );
            }
            _ => {
                println!("     x={x:4.1}: evaluation error");
            }
        }
    }
...
fn performance_summary() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Testing {} evaluations × {} iterations",
        test_points.len(),
        iterations
    );
