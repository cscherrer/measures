...
fn main() {
    ...
    let density1: f64 = normal1.log_density().at(&test_point);
    let density2: f64 = normal2.log_density().at(&test_point);
    println!(
        "Standard densities: p1={:.6}, p2={:.6}, diff={:.6}",
        density1,
        density2,
        density1 - density2
    );
    ...
    let relative_density: f64 = normal1.log_density().wrt(normal2.clone()).at(&test_point);
    let manual_computation = density1 - density2;
    println!(
        "Relative density: {:.6}, manual: {:.6}, diff: {:.10}",
        relative_density,
        manual_computation,
    ...
    let log_bayes_factor: f64 = density1 - density2;

    println!("Applications:");
    println!("  Importance weight: {importance_weight:.6}");
