...
fn demonstrate_normal_auto_jit() {
    ...
    match normal.auto_symbolic() {
        Ok(symbolic) => {
            ...
            println!(
                "  Parameters: {:?}",
                symbolic.parameters.keys().collect::<Vec<_>>()
            );
            ...
            if let Ok(symbolic_result) = symbolic.evaluate_single("x", test_x) {
                ...
                println!(
                    "    Difference:    {:.2e}",
                    (symbolic_result - standard_result).abs()
                );
            }
        }
    ...
    match normal.auto_jit() {
        Ok(jit_fn) => {
            ...
            println!(
                "    Difference:    {:.2e}",
                (jit_result - standard_result).abs()
            );
        }
...
fn compare_auto_vs_manual() {
    ...
    for &x in &test_values {
        ...
        if let Ok(auto_result) = auto_symbolic.evaluate_single("x", x) {
            ...
            println!(
                "  x={x:4.1}: standard={standard:.6}, auto={auto_result:.6}, error={error:.2e}"
            );
        }
    }
...
fn benchmark_auto_jit_performance() {
    ...
    if let Ok(jit_fn) = normal.auto_jit() {
        ...
        println!(
            "  Standard evaluation: {:?} ({:.2} ns/call)",
            standard_time,
            standard_time.as_nanos() as f64 / f64::from(n_iterations)
        );
        println!(
            "  Auto-JIT evaluation: {:?} ({:.2} ns/call)",
            jit_time,
            jit_time.as_nanos() as f64 / f64::from(n_iterations)
        );
