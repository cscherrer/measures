...
#[cfg(feature = "symbolic")]
use measures::{LogDensityBuilder, Normal, exponential_family::ExponentialFamily};
...
fn main() {
    ...
    let term2 = -(Expression::new_val(1.0) / (two.clone() * sigma_squared.clone()))
        * (x.clone() * x.clone());
    ...
    println!(
        "For our distribution Normal(μ={}, σ={}) at x={}:",
        normal.mean, normal.std_dev, x_value
    );

    // Evaluate each term
    if let Some(t1_val) = term1.eval_args(&vars) {
        println!("  Term 1 value: {t1_val:.6}");
    }
    if let Some(t2_val) = term2.eval_args(&vars) {
        println!("  Term 2 value: {t2_val:.6}");
    }
    if let Some(t3_val) = term3.eval_args(&vars) {
        println!("  Term 3 value: {t3_val:.6}");
    }

    // Evaluate the exponential family part
    match exp_fam_part.eval_args(&vars) {
        Some(symbolic_result) => {
            println!("  Exponential family part: {symbolic_result:.6}");
            ...
            let computed_exp_fam =
                natural_params[0] * sufficient_stats[0] + natural_params[1] * sufficient_stats[1];

            println!("  Direct computation:      {computed_exp_fam:.6}");
            println!(
                "  Difference:              {:.10}",
                (symbolic_result - computed_exp_fam).abs()
            );
    ...
    println!(
        "  η₁ = μ/σ² = {}/{:.1}² = {:.6}",
        normal.mean, normal.std_dev, eta1_numeric
    );
    println!(
        "  η₂ = -1/(2σ²) = -1/(2×{:.1}²) = {:.6}",
        normal.std_dev, eta2_numeric
    );

    // Evaluate symbolic natural parameters
    if let (Some(eta1_symbolic), Some(eta2_symbolic)) =
        (eta1.eval_args(&vars), eta2.eval_args(&vars))
    {
        println!("  η₁ (symbolic) = {eta1_symbolic:.6}");
        println!("  η₂ (symbolic) = {eta2_symbolic:.6}");
        println!(
            "  Differences: Δη₁={:.10}, Δη₂={:.10}",
    ...
    {
        println!("Standard Normal (μ=0, σ=1):");
        println!("  η₁ = {eta1_std:.6}");
        println!("  η₂ = {eta2_std:.6}");
    }
    ...
    if let (Some(eta1_hv), Some(eta2_hv)) = (
        eta1.eval_args(&high_var_vars),
        eta2.eval_args(&high_var_vars),
    ) {
