...
fn main() {
    ...
    for (i, samples) in test_cases.iter().enumerate() {
        ...
        if samples.is_empty() {
            ...
            let iid_log_density = iid_normal.iid_log_density(samples);
    ...
    let iid_single = iid_normal.iid_log_density(&single_sample);
    ...
    let joint_density = iid_normal.iid_log_density(&joint_samples);
    ...
    let base_density = iid_normal.iid_log_density(&base_sample);
    let extended_density = iid_normal.iid_log_density(&extended_sample);
