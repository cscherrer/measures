...
fn test_real_bayesian_jit_compilation(data: &[f64]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📝 Compiling posterior: log p(μ|x={}) ∝ log p(x={}|μ) + log p(μ)", data[0], data[0]);
            ...
            match compiler.compile_expression(
                ...
                Ok(jit_function) => {
                    ...
                    for mu in test_mu_values {
                        ...
                        let manual_result = {
                            let x = data[0];
                            let log_likelihood = -0.5 * (x - mu).powi(2) - 0.5 * (2.0 * std::f64::consts::PI).ln();
                            let log_prior = -0.5 * mu.powi(2) - 0.5 * (2.0 * std::f64::consts::PI).ln();
                        ...
                        println!("   μ={mu:.1}: JIT={jit_result:.6}, Manual={manual_result:.6}, Diff={diff:.2e} {status}");
                    }
...
fn test_jit_performance(
    ...
    let test_mu_values: Vec<f64> = (0..1000).map(|i| i as f64 * 0.01).collect();
    ...
    for _ in 0..num_iterations {
        for &mu in &test_mu_values {
            let _result = {
                let log_likelihood = -0.5 * (x_value - mu).powi(2) - 0.5 * (2.0 * std::f64::consts::PI).ln();
                let log_prior = -0.5 * mu.powi(2) - 0.5 * (2.0 * std::f64::consts::PI).ln();
                log_likelihood + log_prior
            };
        }
    ...
    println!("📈 Results ({} calls):", total_calls);
    println!("   JIT:    {:.1} ns/call", jit_ns_per_call);
    println!("   Manual: {:.1} ns/call", manual_ns_per_call);
...
mod extended_jit_demo {
    ...
    impl BayesianModel {
        pub fn new(likelihood_expr: Expr, prior_expr: Expr, data: Vec<f64>) -> Self {
            Self { likelihood_expr, prior_expr, data }
        }
