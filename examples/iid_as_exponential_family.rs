...
fn main() {
    ...
    println!("  IID: [{:.6}, {:.6}]", natural_params[0], natural_params[1]);
    println!("  Base: [{:.6}, {:.6}]", base_natural_params[0], base_natural_params[1]);
    ...
    let individual_stats: Vec<_> = samples.iter()
        .map(|&x| {
    ...
    println!("  IID: [{:.6}, {:.6}]", iid_sufficient_stat[0], iid_sufficient_stat[1]);
    ...
    println!("IID method: {:.6}", log_density_iid);
    ...
    println!("Central IID function: {:.6}", log_density_central);
    
    // Method 3: Manual summation (for verification)
    let log_density_manual: f64 = samples.iter()
        .map(|&x| normal.log_density().at(&x))
        .sum();
    println!("Manual summation: {:.6}", log_density_manual);
    ...
    println!("  IID([x]): {:.6}", single_log_density);
    println!("  Individual: {:.6}", individual_log_density);
