...
fn main() {
    ...
    println!("Separate calls: natural=[{:.6}, {:.6}], log_partition={:.6}, time={:?}",
             natural_params_sep[0], natural_params_sep[1], log_partition_sep, separate_time);
    
    println!("Combined call:  natural=[{:.6}, {:.6}], log_partition={:.6}, time={:?}",
             natural_params_combined[0], natural_params_combined[1], log_partition_combined, combined_time);

    // Verify identical results
    let params_match = (natural_params_sep[0] - natural_params_combined[0]).abs() < 1e-10
    ...
    assert!(params_match && partition_match, "Results should be identical");
    ...
    println!("IID log-density: {:.8}, time: {:?}", iid_result, iid_time);
