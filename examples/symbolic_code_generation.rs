...
//! 4. Compare performance against cached evaluation
//!
//! This approach could potentially replace cache-based optimization by generating
//! specialized functions that are extremely efficient for repeated evaluation.
//! 
...
fn main() {
    ...
    for &x in &[0.0, 1.0, 2.0] {
        if let Some(result) = optimized_normal.evaluate(x) {
            ...
            println!("  x={:.1}: symbolic={:.8}, expected={:.8}, diff={:.2e}", 
...
impl SymbolicLogDensity {
    fn evaluate(&self, x: f64) -> Option<f64> {
        ...
        let vars_str: HashMap<&str, f64> = vars.iter()
            .map(|(k, v)| (k.as_str(), *v))
            .collect();
...
fn performance_comparison(normal: &Normal<f64>, generated: &GeneratedCode) {
    ...
    for &x in &test_points {
        ...
        // Method 2: Cached exponential family computation
        let cache = normal.precompute_cache();
        let cached_result: f64 = normal.cached_log_density(&cache, &x);
        
        // Method 3: Generated optimized code (simulated)
        let optimized_result = generated.evaluate(x);
        
        // Method 4: Real generated function
        let real_optimized_result = optimized_function(x);
        
        println!("  x={:.1}: standard={:.8}, cached={:.8}, optimized={:.8}, real={:.8}", 
                 x, standard_result, cached_result, optimized_result, real_optimized_result);
        
        // Verify all methods agree
        let diff1 = (standard_result - cached_result).abs();
        let diff2 = (standard_result - optimized_result).abs();
        let diff3 = (standard_result - real_optimized_result).abs();
        
        let max_diff = diff1.max(diff2).max(diff3);
        if max_diff < 1e-10 {
            println!("         ✓ All methods agree (max diff: {:.2e})", max_diff);
        } else {
            println!("         ⚠ Methods disagree! Diffs: {:.2e}, {:.2e}, {:.2e}", diff1, diff2, diff3);
        }
    }
    
    println!("\nPerformance characteristics:");
    println!("• Standard:  Recomputes σ², log(2πσ²), etc. each time");
    println!("• Cached:    Precomputes natural params, log partition once");
    ...
    println!("optimized_function({}) = {:.10}", demo_x, result);
    ...
    println!("Timing {} iterations:", n_iterations);
    println!("  Standard:  {:?} ({:.2} ns/call)", standard_time, standard_time.as_nanos() as f64 / n_iterations as f64);
    println!("  Cached:    {:?} ({:.2} ns/call)", cached_time, cached_time.as_nanos() as f64 / n_iterations as f64);
    println!("  Generated: {:?} ({:.2} ns/call)", optimized_time, optimized_time.as_nanos() as f64 / n_iterations as f64);
    ...
    println!("  Speedup vs standard: {:.2}x", speedup_vs_standard);
    println!("  Speedup vs cached:   {:.2}x", speedup_vs_cached);
}
...
fn demonstrate_general_approach() {
    ...
    println!("This could extend the current exponential family framework:");
    println!("• Add SymbolicOptimizer trait");
    println!("• Generate code for fixed-parameter distributions");
    println!("• Fall back to cached computation for dynamic parameters");
...
// Current API
let normal = Normal::new(2.0, 1.5);
let cached_fn = normal.cached_log_density_fn();

// Extended API with symbolic optimization  
let optimized_fn = normal.symbolic_optimize()
    .generate_specialized_code()
    .compile()?;

// Usage is identical, but performance is much better
...
"#);
    ...
    println!("✅ Could replace dynamic caching with static optimization");
...
fn main() {
    ...
    println!("The approach could potentially replace cache-based optimization with");
    println!("generated specialized code that has zero runtime overhead.");
} 