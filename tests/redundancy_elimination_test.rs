...
fn test_natural_params_specified_only_once() {
    ...
    for (cached, direct) in normal_natural_params.iter().zip(normal_direct_params.iter()) {
        let diff: f64 = cached - direct;
        assert!(diff.abs() < 1e-10);
    }
    
    for (cached, direct) in poisson_natural_params.iter().zip(poisson_direct_params.iter()) {
        let diff: f64 = cached - direct;
        assert!(diff.abs() < 1e-10);
    }
}
