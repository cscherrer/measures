...
pub fn log_binomial_coefficient<F: Float>(n: u64, k: u64) -> F {
    ...
    if let (Some(n_f64), Some(k_f64)) = (n.to_f64(), k.to_f64()) {
        let log_coeff = {
            let (ln_gamma_n_plus_1, _) = special::Gamma::ln_gamma(n_f64 + 1.0);
            let (ln_gamma_k_plus_1, _) = special::Gamma::ln_gamma(k_f64 + 1.0);
            let (ln_gamma_n_minus_k_plus_1, _) = special::Gamma::ln_gamma(n_f64 - k_f64 + 1.0);
