...
#[cfg(feature = "jit")]
use measures::exponential_family::jit::{generate_zero_overhead_exp_fam, JITFunction};
...
fn benchmark_normal_distribution(c: &mut Criterion) {
    ...
    group.bench_function("manual_jit", |b| {
        ...
        let jit_fn = compiler.compile_custom_expression(&symbolic).expect("Manual JIT should succeed");
        b.iter(|| {
            for &x in &test_values {
                black_box(jit_fn.call(black_box(x)));
            }
...
fn benchmark_exponential_distribution(c: &mut Criterion) {
    ...
    #[cfg(feature = "jit")]
    group.bench_function("auto_jit", |b| {
        let jit_fn = exponential.auto_jit().expect("JIT compilation should succeed");
        b.iter(|| {
            for &x in &test_values {
                black_box(jit_fn.call(black_box(x)));
            }
...
fn benchmark_compilation_overhead(c: &mut Criterion) {
    ...
    group.bench_function("auto_jit_compilation", |b| {
        b.iter(|| {
            let _jit_fn = black_box(normal.clone()).auto_jit().expect("JIT compilation should succeed");
        });
    });
    ...
    group.bench_function("symbolic_generation", |b| {
        b.iter(|| {
            let _symbolic = black_box(normal.clone()).auto_symbolic().expect("Symbolic generation should succeed");
        });
    });
...
fn benchmark_accuracy_vs_performance(c: &mut Criterion) {
    ...
    let normal = Normal::new(0.0, 1.0);
    let test_values: Vec<f64> = (0..1000).map(|i| (i as f64 - 500.0) / 100.0).collect();
...
fn benchmark_scalability(c: &mut Criterion) {
    ...
    for &size in &sizes {
        let test_values: Vec<f64> = (0..size).map(|i| i as f64 / size as f64).collect();
