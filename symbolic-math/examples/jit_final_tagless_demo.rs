...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..1_000_000 {
        sum += compiled_simple.call_single(f64::from(i));
    }
    ...
    println!(
        ...
        jit_time.as_nanos() as f64 / 1_000_000.0
    );
    ...
    println!(
        "Average per call: {:.2} ns",
        native_time.as_nanos() as f64 / 1_000_000.0
    );
    ...
    if speedup_ratio > 0.5 {
        ...
        println!(
            "📈 JIT has overhead but still provides good performance for complex expressions."
        );
    }
