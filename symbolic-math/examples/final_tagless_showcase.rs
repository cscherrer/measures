...
use symbolic_math::final_tagless::{
    ContextualEval, DirectEval, ExprBuilder, FinalTaglessExpr, MathExpr, PrettyPrint,
    StatisticalExpr,
};
...
fn performance_benchmark() {
    ...
    println!(
        "  AST eval:    {:?} ({:.2} ns/call)",
        ast_time,
        ast_time.as_nanos() as f64 / ITERATIONS as f64
    );

    let speedup = ast_time.as_nanos() as f64 / direct_time.as_nanos() as f64;
    println!("  Speedup: {speedup:.2}x faster with final tagless!");
}
