...
impl<T> crate::exponential_family::symbolic::SymbolicOptimizer<u64, T> for NegativeBinomial<T>
where
    T: Float + std::fmt::Debug + 'static,
{
    fn symbolic_log_density(&self) -> crate::exponential_family::symbolic::SymbolicLogDensity {
        ...
        // Build symbolic log-density: log C(x+r-1,x) + x * log(p/(1-p)) + r * log(1-p)
        let prob_f64 = self.prob.to_f64().unwrap();
        let r_f64 = self.r as f64;
        let log_odds = (prob_f64 / (1.0 - prob_f64)).ln();
        let log_failure_prob = (1.0 - prob_f64).ln();

        // Expression: x * log(p/(1-p)) + r * log(1-p)
        // (negative binomial coefficient will be added separately)
        let expr = x * symbolic_const(log_odds) + symbolic_const(r_f64 * log_failure_prob);

        // Store parameters
        let mut parameters = HashMap::new();
        parameters.insert("r".to_string(), r_f64);
    ...
    fn generate_optimized_function(
        ...
        let log_odds = (prob_f64 / (1.0 - prob_f64)).ln();
        let log_failure_prob = (1.0 - prob_f64).ln();
        let r_log_failure_prob = r_f64 * log_failure_prob;

        // Convert back to T type
        let log_odds_t = T::from(log_odds).unwrap();
        let r_log_failure_prob_t = T::from(r_log_failure_prob).unwrap();
        ...
        let function = Box::new(move |x: &u64| -> T {
            ...
            // Compute negative binomial coefficient
            let neg_binomial_coeff =
                if let (Some(x_f64), Some(r_f64)) = (x.to_f64(), r_copy.to_f64()) {
                    let log_coeff = {
                        let (ln_gamma_x_plus_r, _) = (x_f64 + r_f64).ln_gamma();
                        let (ln_gamma_x_plus_1, _) = (x_f64 + 1.0).ln_gamma();
                        let (ln_gamma_r, _) = r_f64.ln_gamma();
            ...
            neg_binomial_coeff + x_t * log_odds_t + r_log_failure_prob_t
        });

        // Store constants for documentation
        let mut constants = HashMap::new();
        constants.insert("r".to_string(), r_f64);
        ...
        let source_expression = format!(
            "NegativeBinomial(r={}, p={prob_f64}): log C(x+r-1,x) + x * {log_odds} + {r_log_failure_prob}",
            self.r
        );
