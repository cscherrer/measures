...
impl<F> StaticInlineJITOptimizer<F, F> for crate::distributions::continuous::Normal<F>
where
    F: Float + Copy,
{
    fn compile_static_inline_jit(
        ...
        let mu = self.mean.to_f64().unwrap_or(0.0);
        let sigma = self.std_dev.to_f64().unwrap_or(1.0);
        Ok(StaticInlineJITCompiler::compile_normal(mu, sigma))
    }
}

// Implementation for CustomJITOptimizer for Normal distribution
impl<F> CustomJITOptimizer<F, F> for crate::distributions::continuous::Normal<F>
where
    F: Float + Copy,
{
    fn custom_symbolic_log_density(&self) -> CustomSymbolicLogDensity {
        ...
        // Create symbolic expression: -0.5 * ln(2πσ²) - (x - μ)² / (2σ²)
        let x = Expr::Var("x".to_string());
        let mu_val = self.mean.to_f64().unwrap_or(0.0);
        let sigma_val = self.std_dev.to_f64().unwrap_or(1.0);
        let sigma_sq = sigma_val * sigma_val;

        let log_norm = Expr::Const(-0.5 * (2.0 * std::f64::consts::PI * sigma_sq).ln());
        let x_minus_mu = Expr::Sub(Box::new(x), Box::new(Expr::Const(mu_val)));
        let quadratic = Expr::Div(
            Box::new(Expr::Pow(Box::new(x_minus_mu), Box::new(Expr::Const(2.0)))),
            Box::new(Expr::Const(2.0 * sigma_sq)),
        );
        let expr = Expr::Sub(Box::new(log_norm), Box::new(quadratic));

        CustomSymbolicLogDensity::new(expr, std::collections::HashMap::new())
    }
}

// Implementation for Exponential distribution
impl<F> StaticInlineJITOptimizer<F, F> for crate::distributions::continuous::Exponential<F>
where
    F: Float + Copy,
{
    fn compile_static_inline_jit(
        ...
        let lambda = self.rate.to_f64().unwrap_or(1.0);
        Ok(StaticInlineJITCompiler::compile_exponential(lambda))
    }
}
...
mod tests {
    ...
    fn test_normal_custom_jit() {
        ...
        let result = symbolic.evaluate_single("x", 1.5);
        assert!(result.is_ok());

        // Verify it matches the analytical computation
        let expected = normal.log_density().at(&1.5);
